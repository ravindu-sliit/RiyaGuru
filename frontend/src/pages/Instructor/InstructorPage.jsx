      ...
      <div className="max-w-7xl mx-auto px-8 py-10">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 mb-10">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            ...
            <div className="flex flex-wrap gap-3">
              <Link to="/instructors/add" className="bg-orange-500 hover:bg-orange-600 text-white px-6 py-3 rounded-xl font-semibold shadow-md">
                Add Instructor
              </Link>
              <Link to="/instructors/list" className="bg-white hover:bg-slate-50 text-slate-700 px-6 py-3 rounded-xl font-semibold border shadow-sm">
                View List
              </Link>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {loading
            ...
            : cards.map((c, i) => (
                <div key={i} className="group bg-white rounded-2xl shadow-sm border p-6 hover:shadow-lg transition-all cursor-pointer relative overflow-hidden">
                  ...
                  <div className="flex items-center justify-between mb-4 relative z-10">
                    ...
                    <div className={`text-3xl font-bold bg-gradient-to-r ${c.gradient} bg-clip-text text-transparent`}>{c.value}</div>
                  </div>
        ...
        <div className="mt-12 grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="bg-white rounded-2xl shadow-sm border p-8">
            ...
            <div className="space-y-4">
              {updates.map((ins, i) => (
                <div key={i} className="flex justify-between items-center p-4 rounded-lg border hover:bg-slate-50">
                  <div>
                    <h3 className="font-semibold text-slate-800">{ins.name}</h3>
                    <p className="text-sm text-slate-600">{ins.specialization}</p>
                  </div>
              ...
              {updates.length === 0 && <p className="text-slate-500">No recent updates</p>}
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border p-8">
            ...
            <div className="space-y-4">
              <Link to="/instructors/reports" className="flex items-center gap-3 p-4 rounded-xl bg-blue-50 border hover:bg-blue-100 transition">
              ...
              <Link to="/instructors/training" className="flex items-center gap-3 p-4 rounded-xl bg-green-50 border hover:bg-green-100 transition">
                <div className="w-10 h-10 rounded-lg bg-green-100 flex items-center justify-center text-green-600">⚙️</div>
                <div>
                  <h3 className="font-semibold text-slate-800">Training Programs</h3>
                  <p className="text-sm text-slate-600">Manage certifications</p>
                </div>
              </Link>
              <Link to="/instructors/settings" className="flex items-center gap-3 p-4 rounded-xl bg-purple-50 border hover:bg-purple-100 transition">
